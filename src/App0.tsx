...
const DEEPSEEK_API_KEY = "sk-055933b147d84d78893e09e2b432c79a";
...
export function App() {
  ...
  const [evaluationCriteria, setEvaluationCriteria] = useState({
    ...
    "academic_rigour": {
      "criteria": "All key data, models, and conclusions should be properly referenced using recognized academic citation formats. Content must adhere to academic integrity, with accurate technical terms and definitions. Where necessary, include a references or bibliography slide and ensure that figures, tables, or quotations are clearly attributed.",
      "scale": 5
    }
  });
          ...
          messages: [
            {
              ...
              null,
              2
            )}，如果用户无要求，返回默认评估标准;如果用户有要求,你负责根据一下用户需求调整默认评估标准(发生冲突时用户要求优先级更高),并以JSON格式返回评估标准,仅包含text、image、layout、color,academic_rigour五个方面,每个方面包含criteria和scale(scale是一个单独的数值)：\${prompt}`,
            },
          ],
        },
        {
          ...
          try {
            ...
            if (jsonStart !== -1 && jsonEnd !== -1) {
              ...
              const filteredCriteria = {
                ...
                academic_rigour: newCriteria.academic_rigour || evaluationCriteria.academic_rigour,
              };
